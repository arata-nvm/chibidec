...
use crate::{
    ...
    llir::{
        BranchTarget, EdgeKind, Instruction, InstructionOrTerminator, LinearProgram, Terminator,
...
impl CfgRecovery {
        ...
        for block in self.graph.blocks() {
            match &block.terminator().kind() {
                TerminatorKind::Branch {
                    ...
                    edges.push((block.start(), *target, EdgeKind::Unconditional));
                }
                TerminatorKind::ConditionalBranch {
                    target: BranchTarget::Imm(target),
                    ..
                } => {
                    edges.push((block.start(), *target, EdgeKind::TrueBranch));

                    let next_addr = block.terminator().next_addr();
                    if self.addr_to_node.contains_key(&next_addr) {
                        edges.push((block.start(), next_addr, EdgeKind::FalseBranch));
                    }
                }
                TerminatorKind::Ret => {}
            }
    ...
    fn add_edge(&mut self, from_addr: u64, to_addr: u64, edge: EdgeKind) {
        let from_node = self.addr_to_node[&from_addr];
        let to_node = self.addr_to_node[&to_addr];
        self.graph.graph_mut().add_edge(from_node, to_node, edge);
    }
}
